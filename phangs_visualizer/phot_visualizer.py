...
from phangs_data_access.x_access import XAccess
...
class PhotVisualizer(PhotAccess, GasAccess, SpecAccess, XAccess):
    """
    ...
    def __init__(self, target_name=None, phot_hst_target_name=None, phot_hst_ha_cont_sub_target_name=None,
        ...
        XAccess.__init__(self, x_target_name=x_target_name)
        ...
        for idx, obs_type in enumerate(obs_list):
            ...
            if obs_type in ['hst_broad_band', 'hst_ha', 'nircam', 'miri']:
                ...
                if obs == 'hst':
                    ...
                    not helper_func.ObsTools.check_hst_ha_cont_sub_obs(target=self.phot_hst_target_name)):
                        continue
                    band_list = [helper_func.ObsTools.filter_name2hst_band(target=self.phot_hst_target_name,
                ...
                if helper_func.ObsTools.check_astrosat_obs(target=self.phot_astrosat_target_name):
                    astrosat_band_list = helper_func.ObsTools.get_astrosat_obs_band_list(
    ...
    def plot_img_stamps(self, fig, fig_dict, ra, dec, plot_rad_profile=True):
        ...
        if helper_func.ObsTools.check_hst_ha_cont_sub_obs(target=self.phot_hst_ha_cont_sub_target_name):
            if self.check_coords_covered_by_band(telescope="hst", ra=ra, dec=dec,
                                                 band=helper_func.ObsTools.get_hst_ha_band(
    ...
    def compute_compact_apert_corr_photometry(
            self, ra_list, dec_list,
            roi_arcsec, bkg_roi_rad_in_arcsec, bkg_roi_rad_out_arcsec,
        ...
        complete_nircam_band_list = helper_func.ObsTools.get_nircam_obs_band_list(
            target=self.phot_nircam_target_name, version=self.nircam_data_ver)
        complete_miri_band_list = helper_func.ObsTools.get_miri_obs_band_list(
        ...
        # check if the bands are loaded!
        self.load_phangs_bands(band_list=band_list, flux_unit=flux_unit, load_err=True)
        # get observation and instrument list
        obs_list = []
        instrument_list = []
        target_name_list = []
        flux_table_names = []
        ...
        for running_idx, obj_idx, ra, dec in zip(range(len(idx_list)), idx_list, ra_list, dec_list):
            ...
            for band_idx, band in enumerate(band_list):
                ...
                if cutout_size is None:
                    cutout_size = (3 * bkg_rad_out_arcsec_list[band_idx], 3 * bkg_rad_out_arcsec_list[band_idx])
                else:
                    if ((cutout_size[0] < 3 * bkg_rad_out_arcsec_list[band_idx]) |
                            (cutout_size[1] < 3 * bkg_rad_out_arcsec_list[band_idx])):
                        cutout_size = (3 * bkg_rad_out_arcsec_list[band_idx], 3 * bkg_rad_out_arcsec_list[band_idx])
                ...
                if re_centering:
                    ...
                    if detected_peaks is None:
                        ...
                        re_center_dict = phot_tools.SrcTools.re_center_src_world(
                            ...
                            re_center_rad_arcsec=phot_aperture_arcsec_list[band_idx],
                ...
                # # get profile and curve of groth
                # rad, profile, profile_err = phot_tools.ProfileTools.get_rad_profile_from_img(
                #     img=obs_cutout_dict['%s_img_cutout' % band].data,
                #     wcs=obs_cutout_dict['%s_img_cutout' % band].wcs,
                #     ra=ra, dec=dec, max_rad_arcsec=bkg_rad_out_arcsec_list[band_idx] * 1.1,
                #     img_err=obs_cutout_dict['%s_err_cutout' % band].data, img_mask=None, norm_profile=True,
                #     method='exact'
                # )
                ...
                # # now fit gaussians to the slit profiles
                # rad_of_interest_pix = helper_func.CoordTools.transform_world2pix_scale(
                #     length_in_arcsec=bkg_rad_in_arcsec_list[band_idx], wcs=obs_cutout_dict['%s_img_cutout' % band].wcs)
                # (amp_1_list, amp_2_list, mu_list_pix, sig_1_list_pix, sig_2_list_pix, amp_1_err_list,
                #  amp_2_err_list_pix, mu_err_list_pix,
                #  sig_1_err_list_pix, sig_2_err_list_pix) = (
                #     phot_tools.ProfileTools.fit_double_gauss2rad_profiles(
                #         rad_profile_dict=rad_profile_dict['slit_profile_dict'], std_pix=std_pix,
                #         max_val_in_apert=forced_photomerty_dict['apert_max'],
                #         upper_sig_1_fact=4, upper_sig_2_fact=10,
                #         rad_of_interest=rad_of_interest_pix))
                # # now select the best gaussian fit
                # best_gauss_dict = phot_tools.ProfileTools.get_best_double_gaussian_profile_params(
                #     amp_1_list=amp_1_list, amp_2_list=amp_2_list, mu_list=mu_list_pix, sig_1_list=sig_1_list_pix,
                #     sig_2_list=sig_2_list_pix,
                #     peak_acceptance_rad_pix=std_pix * 1,
                ...
                # mean_sig_1_arcsec = helper_func.CoordTools.transform_pix2world_scale(
                #     length_in_pix=best_gauss_dict['mean_sig_1'], wcs=obs_cutout_dict['%s_img_cutout' % band].wcs)
                # mean_sig_2_arcsec = helper_func.CoordTools.transform_pix2world_scale(
                #     length_in_pix=best_gauss_dict['mean_sig_2'], wcs=obs_cutout_dict['%s_img_cutout' % band].wcs)
                # # get the correction factor
                # # for this purpose we compute an over sampeled double gaussian function
                ...
                # dummy_rad_arcsec = np.linspace(-bkg_rad_out_arcsec_list[band_idx], bkg_rad_out_arcsec_list[band_idx],
                #                                n_pixels_in_bkg_rad * 10)
                # mean_double_gauss = helper_func.FitTools.super_pos_two_gaussian_func(
                #     amp_1=best_gauss_dict['mean_amp_1'], amp_2=best_gauss_dict['mean_amp_2'], mu=mean_mu_arcsec,
                #     sig_1=mean_sig_1_arcsec, sig_2=mean_sig_2_arcsec,
                #     x_data=dummy_rad_arcsec)
                # # compute curve of groth
                # mask_positive_side_double_gaussian = dummy_rad_arcsec > 0
                # cog_double_gaussian = np.cumsum(mean_double_gauss[mask_positive_side_double_gaussian])
                # # norm to max value
                # cog_double_gaussian /= np.max(cog_double_gaussian)
                # # interpolate function
                # cog_double_gaussian_interpolated = interp1d(dummy_rad_arcsec[mask_positive_side_double_gaussian],
                #                                             cog_double_gaussian, kind='cubic')
                # # the correction factor is the inverse of the cod at the radius of the aperture
                # correction_fact = 1 / cog_double_gaussian_interpolated(phot_aperture_arcsec_list[band_idx])


                # now fit gaussians to the slit profiles
                # rad_of_interest_pix = helper_func.CoordTools.transform_world2pix_scale(
                #     length_in_arcsec=(phot_aperture_arcsec_list[band_idx] + bkg_rad_in_arcsec_list[band_idx]) / 2,
                #     wcs=obs_cutout_dict['%s_img_cutout' % band].wcs)
                rad_of_interest_pix = helper_func.CoordTools.transform_world2pix_scale(
                ...
                best_gauss_dict = phot_tools.ProfileTools.get_best_gaussian_profile_params(
                    ...
                    peak_acceptance_rad_pix=std_pix * 1, n_good_fits_needed=4)
                ...
                if plot_flag:
                    ...
                    plotting_tools.StrTools.display_text_in_corner(
                        ax=ax_img, text=band.upper(), fontsize=fontsize_large_label, text_color='tab:red', path_eff_color='k', )
                    ...
                    # ax_profile.plot(dummy_rad_arcsec[mask_positive_side_double_gaussian],
                    #                 cog_double_gaussian * max_rad_pofile,
                    #                 linewidth=4, color='tab:orange')
                    # ax_profile.scatter(phot_aperture_arcsec_list[band_idx], (1 / correction_fact) * max_rad_pofile,
                    #                    s=200, color='k')
                    ...
                    # ax_profile.plot([bkg_rad_in_arcsec_list[band_idx], bkg_rad_out_arcsec_list[band_idx]],
                    #                 [forced_photomerty_dict['bkg_median'],
                    #                  forced_photomerty_dict['bkg_median']],
                    ...
                    ax_slit_profiles.plot([phot_aperture_arcsec_list[band_idx], phot_aperture_arcsec_list[band_idx]],
                    ...
                    ax_sed.errorbar(mean_band_wavelength, forced_photomerty_dict['src_flux'] * corr_fact * gal_red_corr_fact,
                                    xerr=[[mean_band_wavelength - min_band_wavelength],
                                          [max_band_wavelength - mean_band_wavelength]],
                ...
                # correct for flux
                flux_table['%s_flux' % band][running_idx] = forced_photomerty_dict['src_flux'] * corr_fact * gal_red_corr_fact
                flux_table['%s_flux_err' % band][running_idx] = forced_photomerty_dict['src_flux_err'] * corr_fact * gal_red_corr_fact
            ...
            if plot_flag:
                ...
                if obj_name is not None:
                    plotting_tools.StrTools.display_text_in_corner(y_frac=0.95,
                        ax=ax_sed, text=obj_name.upper(), fontsize=fontsize_large_label + 10, text_color='dimgray', path_eff_color='k', )
